...
public class Main {
    ...
    public static String[] makeData(String input) {
        ...
        if (result.length > dataLenght) {
            throw new MoreDateExeption();
        }
        if (result.length < dataLenght) {
            throw new LastDataExeption();
        }
        return result;
    }
    ...
    public static void writeData(String[] result) {
        ...
        try (FileWriter writer = new FileWriter(String.format("Homeworks/Work3/%s.txt", fileName), true)) {
